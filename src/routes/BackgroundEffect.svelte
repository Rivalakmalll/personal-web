<script lang="ts">
	...
	function setBackground() {
		...
		if (hour >= 5 && hour < 12) {
			...
			color = 'linear-gradient(to bottom, #FFD54F, #FFFAFA)'; // Light yellow to white
		} else if (hour >= 12 && hour < 17) {
			// Afternoon (12 PM - 4:59 PM)
			color = 'linear-gradient(to bottom, #FF9800, #FFF3E0)'; // Orange to light orange
		} else if (hour >= 17 && hour < 19) {
			// Evening (5 PM - 6:59 PM)
			color = 'linear-gradient(to bottom, #FF5722, #FFCCBC)'; // Deep orange to peach
		} else {
			// Night (7 PM - 4:59 AM)
			color = 'linear-gradient(to bottom, #1E1E1E, #3D3D3D)'; // Dark gray to light gray
		}

		refDiv.style.background = color;
	}

	onMount(() => {
		ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
		resize();
		setBackground(); // Set initial background based on the current time
		loop();
